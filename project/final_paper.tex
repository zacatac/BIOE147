...
\title{Characterization and Application of trans-cleaving hammerhead ribozymes in genetic circuits} % For titles, only capitalize the first letter
...
\author{Ryan Tsoi\affil{1}{University of California, Berkeley}\affil{2}{Crusher of Hearts, Breaker of Dreams},
...
\begin{figure}
  \centering
  \includegraphics[scale=0.45]{cis_trans.jpeg}
  \caption{Depiction of the action of a trans-cleaving ribozyme}
\end{figure}
...
consistent with the classic toggle circuit. In the RNA circuit (See Fig. 2,3(Table 1),4) , 
...
\caption{ Representative chemical scheme of toggle switch based on two trans-cleaving 
hammerhead ribozymes. Ribozyme A cleaves ribozyme B and vice versa after 
reversibly binding to one another. Translation is not depicted and is assumed to 
have no effect on transcription nor riboregulation in the circuit.}
\end{figure}
\begin{figure}
...
\caption{Table 1. Approximated parameters of individual chemical reactions within the 
riboregulated RNA toggle switch. Values represent rate constants for the reactions 
and are averaged in units of molecules/s for purposes of Gillespie stochastic modeling. 
Constants are calculated based on constants for Escherichia coli bacteria.}     
...
\begin{figure}    
  ...
  \caption{Timecourse of RNA-based toggle switch with two ribozymes from Gillespie 
  algorithm. Given an initial concentration of ARNA, concentration does not 
  reach some steady state value and eventually drops to zero molecules after 10
  hours. BRNA remains low and ABRNA seems to stay constant throughout the reaction
  despite high initial concentratinos of ARNA, implying high amounts of degradation of
  ARNA at early times. G is concentration of reporter protein GFP which is encoded 
  on a separate mRNA strand that is cleaved by ribozyme A.  When A is not present, GFPRNA is free to be translated and thus fluoresce.}
...
Comparing the RNA repressilator to the classic repressilator (see Figure 5,6(Table 2),7), the 
...
\caption{  Representative chemical scheme of repressilator based on three trans-cleaving 
hammerhead ribozymes in a rock-paper-scissor interaction. Ribozyme A binds to
 and cleaves ribozyme B, ribozyme B binds to and cleaves ribozyme C, and 
ribozyme C binds to and cleaves ribozyme A. Translation is not depicted and
 is assumed to have no effect on transcription nor riboregulation in the
 circuit.}
\end{figure}
\begin{figure}
\scalebox{0.7}{
...
$k_{atc}$ & $A_{DNA}\rightarrow A_{RNA}$ & 0.6897\\
$k_{btc}$ & $B_{DNA}\rightarrow B_{RNA}$ & 0.6897\\
$k_{ctc}$ & $C_{DNA}\rightarrow C_{RNA}$ & 0.6897\\
$k_{a,deg}$ & $A_{RNA} \rightarrow \emptyset$ & 0.0033\\
$k_{b,deg}$ & $B_{RNA} \rightarrow \emptyset$ & 0.0033\\
$k_{c,deg}$ & $C_{RNA} \rightarrow \emptyset$ & 0.0033\\
$k_{ab,on}$ & $A_{RNA}+B_{RNA} \rightarrow A_{RNA}\cdot B_{RNA}$ & 101.4\\
$k_{bc,on}$ & $B_{RNA}+C_{RNA} \rightarrow B_{RNA}\cdot C_{RNA}$ & 101.4\\
$k_{ca,on}$ & $C_{RNA}\cdot A_{RNA} \rightarrow C_{RNA}+A_{RNA}$ & 101.4\\
$k_{ab,off}$ & $A_{RNA}\cdot B_{RNA} \rightarrow A_{RNA}+B_{RNA}$ & 0.05\\
$k_{bc,off}$ & $B_{RNA}\cdot C_{RNA} \rightarrow B_{RNA}+C_{RNA}$ & 0.05\\
$k_{ca,off}$ & $C_{RNA}\cdot A_{RNA} \rightarrow C_{RNA}+A_{RNA}$ & 0.05\\
$k_{cleave,a}$ & $A_{RNA}\cdot B_{RNA} \rightarrow A_{RNA}$ & 0.5\\
$k_{cleave,b}$ & $B_{RNA}\cdot C_{RNA} \rightarrow B_{RNA}$ & 0.5\\
$k_{cleave,c}$ & $C_{RNA}\cdot A_{RNA} \rightarrow C_{RNA}$ & 0.5\\
\end{tabular}
}

\caption{Table 2. Approximated parameters of individual chemical reactions 
within the riboregulated RNA repressilator. Values represent rate constants for 
the reactions and are averaged in units of molecules/s for purposes of Gillespie 
stochastic modeling. Constants are calculated based on constants for Escherichia 
coli bacteria.}
\end{figure}
...
\caption{
Average trace over 100 seconds of RNA-based repressilator with three ribozymes
 from Gillespie algorithm. Graph is based on an average sample of 500
 repetitions of timecourses from stochastic model. Time and molecules are both
 kept at low levels in order to clearly observe the oscillations in concentrations. While
 oscillations are observed, they are irregular and unstable, presumably due to high rates of transcription and degradation of mRNA.}
\end{figure}
...
The ability to create reliable, orthogonal, and predictable genetic circuits is 
vital to transforming synthetic biology into an engineering science instead of an 
inefficient technique. The use of riboregulation in mRNA for translational is an example 
of a circuit design that is easily tunable with reaction rates, orthogonal with 
unique Watson-Crick base pairing, and designable in comparison to protein designing. 
Establishing RNA as superior to proteins for meeting the design principles of genetic 
engineering has a profound effect on circuit design from both a bottom-up and top-down 
design methodology and can solve the issue of scaling system complexity.

Here, we modeled RNA gene circuit equivalents of two well-characterized systems, 
the toggle switch and the repressilator, and compared outputs of each system to their 
respective counterparts using traditional promoter-repressor pairings. At the core 
of this system is the trans-cleaving hammerhead ribozyme, which recognizes a sequence 
of ribonucleotide bases on an mRNA strand, binds to that site, and cleaves the mRNA. 
When two ribozymes that cleave one another are placed together in a circuit, we achieve 
an effect similar to a toggle switch where high transcription of one ribozyme leads to 
constant high concentratinos of that ribozyme.  In addition, when three ribozymes are 
placed together in a rock-paper-scissors fashion, we effectively obtain a repressilator, 
where concentrations of each ribozyme vary with some set period of oscillation.

The toggle switch obtained using riboregulation did not obtain the desired output 
as degradation rates dominated the system and eventually led to a deterministic 
outcome where neither ribozyme is present in the system. When modeling the RNA repressilator, 
concentration oscillations for each species are clearly seen from the stochastic model. 
However, high rates of transcription and translation introduce noise into the system, 
leading to irregular time periods as well as amplitudes for each oscillation. 
In addition, the system never reaches a state where any of the ribozymes reach a 
significantly low concentration as all of the ribozymes vary from 0.5-0.8 molecules 
during the course of the simulation. Riboregulation of mRNA has been observed to be 
an effective method for genetic circuit design ~\cite{Samarsky}, and it is likely that with 
additional tuning an effective repressilator could be coded using RNA-based logic.
...
in these systems. To overcome this challenge one could have designed the hammerhead
ribozyme to have some kind of cooperative allosteric binding. 
However, design of a new hammerhead ribozyme 
that somehow dimerized before its cleaving action was activated was counter to the
robust and self-contained system that we were trying to implement. The need for 
simplicity in each of the individual riboregulatory elements seemed to be a more important
design decision for future applications than having a more complex, but cooperative
riboregulatory element. 

Despite the challenges faced during modeling, we believe RNA still provides 
more promise for achieving predictable, orthogonal, reliable and safe designs than proteins. 
If RNA circuits can be optimized and a system of parts established, it will revolutionize biological 
circuit design and allow synthetic biology to rapidly grow and become an established engineering science.
...
the same set of tools, and the same process. The representative chemical reactions of each 
of these circuits designed by hand in a way that was equivalent to the original
toggle switch (Fig. 2) and repressilator design (Fig. 5). Each of these chemical
...
terminates upon the arrival at the endtime. The species are tracked throughout this time
course and can then be plotted to give a time trace of their changing concentrations.
...
\bibitem{Samarsky}Samarsky et al.; \emph{A small nucleolar RNA:ribozyme hybrid cleaves a nucleolar RNA
